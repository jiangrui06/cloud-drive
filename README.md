...
企业云盘管理系统是一个功能完善、安全高效的企业级文件存储与协作平台。系统采用 B/S 架构，支持多种登录方式、用户与群组管理、文件秒传、在线编辑、文件分享等核心功能，同时提供完善的操作审计与权限控制机制，满足企业对数据安全与高效协作的双重需求。
...
### 用户与群组管理
- **用户管理**：管理员可创建、编辑、禁用/启用用户
- **角色管理**：支持管理员/普通用户双角色体系
- **群组管理**：创建群组、成员管理、角色分配（群主/管理员/成员）
...
│   │   └── db.js                 # 数据库连接池
│   └── routes/                    # 路由处理
│       ├── auth.js               # 认证路由(注册、登录、Token刷新)
│       ├── users.js              # 用户管理路由(CRUD、日志查询)
│       ├── groups.js             # 群组管理路由(创建、成员管理)
│       └── files.js              # 文件管理路由(核心功能)
├── public/                        # 前端静态文件
│   ├── index.html                 # 首页/登录页
│   ├── login.html                 # 登录/注册页
│   ├── dashboard.html             # 工作台仪表盘
│   ├── file-manager.html          # 文件管理器
│   ├── online-edit.html           # 在线编辑器
│   ├── share-expired.html         # 分享失效页
│   ├── css/
│   │   └── style.css             # 全局样式
│   ├── js/
│   │   ├── api.js                # API 请求封装
...
1. 用户上传文件时，前端计算文件部分内容的哈希值
2. 系统首先查询数据库中是否存在相同 MD5 且相同大小的文件
...
| POST   | `/api/files/share/:id` | 生成分享链接 |
| GET    | `/api/files/search` | 搜索文件 |
| GET    | `/api/files/recent` | 最近文件 |
| GET    | `/api/files/storage/stats` | 存储统计 |
...
| POST   | `/api/groups/:id/members` | 管理成员 |
...
# 启动服务
pm2 start server/app.js --name cloud-drive
...
1. 修改默认管理员密码
2. 配置 HTTPS 证书
3. 修改 JWT Secret 为随机字符串
4. 配置数据库访问白名单
5. 启用验证码功能（取消 config.js 中 captcha.enabled 的注释）
6. 定期备份数据库
7. 配置防火墙限制访问端口
